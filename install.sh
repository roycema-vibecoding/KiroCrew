...
_bootstrap_python() {
    if ! has mise; then
        info "No Python $PYTHON_VERSION from the system; installing mise…"
        curl -fsSL https://mise.run | sh >/dev/null 2>&1 || true
        export PATH="$HOME/.local/bin:$PATH"
        has mise || return 1
    fi
    info "Provisioning Python $PYTHON_VERSION via mise…"
...
if ! _find_python; then
    ...
    if [ "$(uname)" = "Darwin" ]; then
        ...
        _find_python || _bootstrap_python \
            || die "Python install failed. Run: sudo apt-get install -y python3.12 python3.12-venv"
    elif has dnf; then
        ...
        _find_python || _bootstrap_python \
            || die "Python install failed. Run: sudo dnf install -y python3.12"
    elif has yum; then
        ...
        _find_python || _bootstrap_python \
            || die "Python 3.12+ could not be installed or provisioned. Install it and re-run."
    elif has brew; then
        ...
        _bootstrap_python \
            || die "Python 3.12+ required but not found, no package manager detected,
     and provisioning one via mise failed.
     Install Python 3.12+ manually (https://www.python.org/downloads/) and re-run."
    fi
fi
