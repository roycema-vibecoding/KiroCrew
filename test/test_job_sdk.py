...
class TestAnUndrivenResultIsFailedNotDone:
    ...
    def test_a_sync_runner_returning_a_coroutine_is_failed(self, sdk: JobSDK) -> None:
        ...
        assert "never began" in run.error
    ...
    def test_a_returned_suspended_generator_is_failed(self, sdk: JobSDK) -> None:
        ...
        assert run.status == FAILED
        assert "suspended partway" in run.error
    ...
    def test_a_returned_never_started_generator_is_failed(self, sdk: JobSDK) -> None:
        """The other failing state, with its OWN reason.

        Creating a generator runs no body, so a runner returning one it never stepped
        did no work -- as lazy as calling an `async def`. Distinct from suspended,
        because "never ran" and "stopped partway" are different facts.
        """
        ...
        assert run.status == FAILED
        assert "never began" in run.error
        assert "suspended partway" not in run.error

    def test_a_returned_drained_generator_is_not_treated_as_failure(self, sdk: JobSDK) -> None:
        """The one allowed frame state: closed, so it ran to completion."""

        def wrapper(handle: JobHandle):
            gen = (n for n in range(3))
            list(gen)  # drained: frame is now None
            return gen

        sdk.register("gen-drained", wrapper)
        assert _wait_terminal(sdk, sdk.start("gen-drained")).status == DONE

    def test_every_frame_state_the_language_defines_has_a_verdict(self) -> None:
        """Completeness asserted BY NAME over the language's own state set.

        The argument for this classifier being complete is that Python defines these
        states and every one is mapped -- so the test names them rather than making a
        reader count branches. An earlier revision had two suspendable outcomes
        because the discussion had two, and silently allowed a suspended frame.
        """
        for name, _is_kind, _state_of, verdicts in job_sdk._SUSPENDABLE_PROTOCOLS:
            if name == "generator":
                states = {
                    inspect.GEN_CREATED: "never began",
                    inspect.GEN_RUNNING: "suspended partway",
                    inspect.GEN_SUSPENDED: "suspended partway",
                    inspect.GEN_CLOSED: "",
                }
            else:
                states = {
                    inspect.CORO_CREATED: "never began",
                    inspect.CORO_RUNNING: "suspended partway",
                    inspect.CORO_SUSPENDED: "suspended partway",
                    inspect.CORO_CLOSED: "",
                }
            assert set(verdicts) == set(
                states
            ), f"{name}: unmapped states {set(states) ^ set(verdicts)}"
            for state, expected in states.items():
                verdict = verdicts[state].format(name=name, state=state)
                if expected == "":
                    assert verdict == "", f"{name}/{state} should be allowed, got {verdict!r}"
                else:
                    assert expected in verdict, f"{name}/{state}: {verdict!r} lacks {expected!r}"
    ...
    def test_an_unrecognised_frame_state_fails_rather_than_passing(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """A future Python adding a state must not become a silent `done`.

        Driven through `_undriven_result` itself rather than by formatting the
        template, because the branch under test is the lookup's DEFAULT -- a mutation
        changing it to allow survived a test that only checked the template.
        """
        sentinel = (x for x in [1])
        monkeypatch.setattr(
            job_sdk,
            "_SUSPENDABLE_PROTOCOLS",
            (
                (
                    "generator",
                    inspect.isgenerator,
                    lambda _obj: "GEN_TELEPORTED",  # a state this build cannot know
                    {inspect.GEN_CLOSED: ""},
                ),
            ),
        )
        verdict = job_sdk._undriven_result(sentinel)
        assert verdict != "", "an uninterpretable state was allowed through as done"
        assert "GEN_TELEPORTED" in verdict, verdict

    def test_the_three_states_are_reachable_and_distinct(self) -> None:
        """The states are not hypothetical: build one of each and read it back."""
        ...
        assert len(verdicts) == 3, f"the three states were flattened: {verdicts}"
        assert job_sdk._undriven_result(closed) == ""
    ...
    def test_every_result_shape_in_the_table_has_a_verdict(self) -> None:
        ...
        table: list[tuple[str, object, str]] = [
            ...
            ("frame: never began (coroutine)", a_coroutine, "never began"),
            ("frame: never began (generator)", unstarted_gen, "never began"),
            ("frame: suspended partway", started_gen, "suspended partway"),
            ("frame: closed, ran to completion", drained_gen, ""),
            ...
            ("neither, no loop here to drive it", an_async_gen, "never ran"),
            ("neither, but still needs driving", bare_awaitable, "never ran"),
        ]
        try:
    ...
    def test_the_two_protocol_questions_cover_the_space(self) -> None:
        ...
        try:
            ...
            for obj in (c, g, loop.create_future(), cf):
                ...
                answers_begun = any(
                    is_kind(obj) for _n, is_kind, _s, _c in job_sdk._SUSPENDABLE_PROTOCOLS
                )
